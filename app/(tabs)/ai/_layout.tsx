...
const TABS = [
  { title: "chat", icon: "home-outline", iconActive: "person" },
...
const renderCustomTabView = (
  props: React.JSX.IntrinsicAttributes & MyTabBarProps
) => <MyTabBar {...props} tabs={TABS} />;

export default function ScrollableMaskedTabView({
  lightColor,
  darkColor,
}: MyTabBarProps) {
  const tabBarBackgroundColor = useThemeColor(
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: tabBarBackgroundColor }}>
      <MaterialTopTabs
        tabBar={(props) => renderCustomTabView({ ...props, tabs: TABS })}>
        {TABS.map((tab) => (
